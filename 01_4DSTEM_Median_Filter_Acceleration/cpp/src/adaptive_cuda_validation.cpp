    ...
    try {
        ...
        const auto split_cuda_diagnostic =
            phase_b::adaptive_median_s3_smax7_cuda_split_diagnostics(
                input.array.data, dimensions);
        const auto balanced_cuda_diagnostic =
        ...
        const auto split_cuda = phase_b::adaptive_median_s3_smax7_cuda_split(
            input.array.data, dimensions);
        const auto balanced_cuda = phase_b::adaptive_median_s3_smax7_cuda_split_balanced(
            input.array.data, dimensions);
        ...
        const std::size_t balanced_cuda_serial = mismatches(
            balanced_cuda_diagnostic.output, serial);
        const std::size_t balanced_cuda_split = mismatches(
            balanced_cuda_diagnostic.output, split_cuda_diagnostic.output);
        const std::size_t split_cuda_diagnostic_difference = mismatches(
            split_cuda.output, split_cuda_diagnostic.output);
        const std::size_t balanced_cuda_diagnostic_difference = mismatches(
            balanced_cuda.output, balanced_cuda_diagnostic.output);
        ...
        const std::size_t balanced_diagnostic_difference = statistic_mismatches(
            cpu_diagnostic.statistics, balanced_cuda_diagnostic.statistics);
        ...
        std::cout << "Compared values: " << input.element_count << '\n'
                  ...
                  << "Balanced CUDA vs optimized serial mismatches: "
                  << balanced_cuda_serial << '\n'
                  << "Balanced CUDA vs split CUDA mismatches: "
                  << balanced_cuda_split << '\n'
                  << "Split CUDA normal vs diagnostic mismatches: "
                  << split_cuda_diagnostic_difference << '\n'
                  << "Balanced CUDA normal vs diagnostic mismatches: "
                  << balanced_cuda_diagnostic_difference << '\n'
                  ...
                  << "Balanced CUDA vs CPU diagnostic-counter differences: "
                  << balanced_diagnostic_difference << '\n'
        ...
        const bool success = baseline_reference == 0 && serial_reference == 0 &&
            ...
            split_cuda_baseline == 0 && balanced_cuda_serial == 0 &&
            balanced_cuda_split == 0 && split_cuda_diagnostic_difference == 0 &&
            balanced_cuda_diagnostic_difference == 0 &&
            diagnostic_difference == 0 && split_diagnostic_difference == 0 &&
            balanced_diagnostic_difference == 0 && input_changes == 0;
