...
Compared with the prior 46-register, 392-byte-stack monolithic adaptive kernel (65.75% achieved occupancy), local-memory work and its long-scoreboard/local-global-throttle stalls have disappeared. The old profile showed 84.91% no-eligible-warp cycles versus 85.89% now: higher occupancy did not cure poor scheduler eligibility. Old/new L1 hit rates were 65.56%/7.02%, L2 hits 97.81%/91.89%, and L1TEX/L2 throughput 74.70%/92.97% versus 11.07%/15.13%; these differences largely reflect removal of stack/local traffic and replay-sensitive measurements, not a proven worsening of the gather. Ranked next candidates were (1) balance the nine-value min/max reduction while leaving gathering and selection untouched, to test the largest sampled dependency site with moderate implementation risk; (2) consider a lower-dependency median-of-nine network, targeting compare/swap waits but with greater correctness risk; (3) adjust gathering/address coalescing, where modest sector excess and low DRAM use imply a lower likely payoff. Candidate 1 was selected for the isolated experiment below; this profiling checkpoint changed no kernel code.

### Phase B balanced common-kernel min/max — 2026-09-23

The retained split common kernel remains unchanged and callable. A separate candidate gathers the same nine values in the same order, then reduces four independent pairs through a balanced tree and folds in the ninth value. Each tie retains the earliest window value, including signed-zero ties for finite input. The 19-comparator median network, boundary/Stage A/B/flag logic, fallback kernel, plane-minimum pass, mapping, and transfer behavior are unchanged. Public 196-value, ignored local 4,096-value, representative 143,360-output, and canonical 47,228,125-output comparisons were bitwise exact against optimized CPU and the retained split CUDA path. All ten diagnostic counters matched; public/local one-shot and diagnostic forms agreed; subset/canonical inputs remained unchanged. Phase A and Phase B CPU/OpenMP regressions passed.

The normal CUDA 12.9 `sm_89` Release A/B uses persistent device buffers, plane minima prepared once, five warm-ups per kernel, and seven individually CUDA-event-timed common launches per version with alternating order; fallback and transfers are outside timing. The final-build primary control runs were `16.343872, 17.260511, 17.276928, 17.254400, 17.236992, 17.203199, 17.106943` ms (median/min/max `17.236992 / 16.343872 / 17.276928`). Candidate runs were `15.325984, 15.504384, 15.869760, 15.799296, 15.849472, 15.855616, 15.699840` ms (median/min/max `15.799296 / 15.325984 / 15.869760`): `1.090997×`, an `8.340756%` reduction, and `2,989.255 Moutput/s` versus `2,739.928 Moutput/s` for the control. Five additional complete sequences yielded `6.63–8.70%` median reductions; one had severe within-sequence drift (~27% CV), so absolute current times are not substituted for the earlier 14.055424 ms historical common-kernel reference. The fresh paired control is the comparison.

Compiled resources changed from 40 to 46 registers/thread, while both kernels retained zero compiled stack/local storage and no SASS local loads/stores. A focused 19-pass Nsight Compute comparison of one warmed launch each measured theoretical/achieved occupancy of `100% / 95.07%` for control and `83.33% / 80.22%` for candidate. Despite that occupancy cost, no-eligible-warp cycles changed only `85.88% → 85.26%`; L1TEX-throttle cycles per issued instruction fell `56.82 → 39.00`, short scoreboard rose `19.02 → 20.51`, and FP64-pipeline utilization stayed near `84.16%`. Min/max source sites remained heavily sampled; inlining and sampled-PC attribution do not prove which upstream instruction caused a stall. The measured gain outweighs the resource cost, so the balanced candidate is retained as a named path. It served as the control for the subsequent median-network test. Profiler-run wall times are not benchmark results; normal non-lineinfo Release settings were restored.

### Phase B lower-dependency median network — rejected — 2026-09-24

One separate candidate replaced only the balanced common kernel's 19-comparator, depth-nine median selector with a 20-comparator, depth-seven median-output network pruned from [Bert Dobbelaere's nine-input sorting network](https://bertdobbelaere.github.io/sorting_networks.html). These are structural comparator depths, not compiled instruction latencies. A selector-local mixed-signed-zero guard used the retained selector when necessary to preserve bitwise tie behavior. The candidate passed all 362,880 distinct-rank permutations, three duplicate patterns, all 262,144 windows over `{-1,-0,+0,+1}`, and a signed-zero Stage B output case. Public (196), ignored local (4,096), representative (143,360), and canonical (47,228,125) outputs matched optimized CPU and balanced CUDA bit for bit; all ten diagnostic counters and input immutability checks passed, as did Phase A and Phase B CPU/OpenMP regressions.
